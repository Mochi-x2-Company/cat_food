...
void gnome_sort(Data arr[], int n, int *ct_c, int *ct_s) {
  int k = 1;    // 反復変数
  while ( k < n ) {                                     // カーソルが末尾に達するまで
    if ( arr_ord_ct(arr, k-1, k, ct_c) ) { k++; continue; }    // 正順ならカーソルを前進
    arr_swap_ct(arr, k-1, k, ct_s);                              // 逆順なら交換
    if ( k > 1 ) { k--; } else { k++; }                 // 逆順なら先頭でない限り後退(先頭では前進)
  }
}
...
{
  int k;
  for ( k = 0; k < n; k++ ) { printf("%03d ", arr[k]); }
}
