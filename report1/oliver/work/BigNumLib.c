...
Bool bignum_input(BigNum *b) 
{
    ...
    //----  節数の入力と確認
    scanf("%d", &u);                    // 節数の入力
    if ( u > UNI ) { return FALSE; }    // 範囲超過なら打切
...
void bignum_output(const BigNum b) 
{
    int p = b.nsz-1;                 // 最上位節
    printf("%3d : ", b.nsz);
    printf("%lld ", b.node[p]);       // 最上位桁の出力(0無)

    while ( --p >= 0 ) {             // 下位桁の出力(0付)
        printf("%0*lld ",LEV , b.node[p]);    // 途中桁の0は出力
    }
    puts("");                        // 出力後の改行
}
...
Bool bignum_init(BigNum *b0, int a, int p) 
{
    ...
    //----  計算処理
    for ( k = 0; k < UNI; k++ ) { b0->node[k] = 0; }    // 各節を0を格納
...
Bool bignum_size(BigNum *b) 
{
    ...
    //----  桁数の計算と格納
    b->dsz = k * LEV;        // node[k-1]〜node[0]は、LEV桁分ある
    v = b->node[k];          // 最上位桁node[k]は、1〜LEV桁しかない
...
Bool bignum_shift(BigNum *b, int s)
{
    ...
    if ( b->nsz + s > UNI ) { return FALSE; }    // 左移動で節溢れ

    //----  右移動(節下げ)
    if ( s < 0 ) {
        for ( k = 0; k < UNI; k++ ) { 
            if ( k-s < UNI ) { b->node[k] = b->node[k-s]; } 
                else { b->node[k] = 0; }
       }
    }
